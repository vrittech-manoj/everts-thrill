...
class MeetTeam(models.Model):
    ...
    def clean(self):
        # Check if another object with the same index exists
        if MeetTeam.objects.filter(index=self.index).exclude(id=self.id).exists():
            raise ValidationError(
                _('An object with index %(index)s already exists. Please choose a different index.'),
                params={'index': self.index},
            )

    def save(self, *args, **kwargs):
        # Call clean method to check for index validation
        self.clean()
