...
class Payment(models.Model): #payment products
    order = models.ForeignKey(DestinationBook,related_name = "payment",on_delete = models.SET_NULL,null = True)
...
class PaymentFail(models.Model):
    order = models.ForeignKey(DestinationBook,related_name = "payment_fail",on_delete = models.SET_NULL,null = True)
    payment_mode = models.CharField(max_length = 100,choices = (('khalti','Khalti'),('esewa','Esewa'),('cod','Cash On Ddelivery')))
